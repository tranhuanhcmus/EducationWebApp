...
BEGIN
	START TRANSACTION;
		SELECT C.*, A.NAME AS AUTHOR 
        FROM COURSE C JOIN ACCOUNT A ON C.OWNERID = A.ID  
		WHERE C.CID IN (SELECT CID FROM CART WHERE ID = UID AND STATUS = true);
    COMMIT;
END $$
DELIMITER ;
...
BEGIN
	START TRANSACTION;
		SELECT C.CMTID, C.FID, C.CONTENT, C.CMT_TIME, A.NAME, (SELECT NAME FROM ACCOUNT ACC JOIN FORUM F ON F.ID = ACC.ID WHERE F.FID = FID) AS AUTHOR, (SELECT F.ID FROM FORUM F WHERE F.FID = FID) AS AUTHORID
